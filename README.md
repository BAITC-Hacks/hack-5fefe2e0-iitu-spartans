...
Ожидаемый результат: `Test Files 11 passed`, `Tests 89 passed`, проверка типов без ошибок. В их числе — сверка
словаря терминов с данными набора в `data/kit` (раздел 9).
...
подсказка (issue #20). Код — `packages/core/src/stt/`, точка подключения — `@voice-router/core/stt`.
...
Результаты замера (`gpt-transcribe`, синтез речи `gpt-4o-mini-tts`, 15 реплик):
...
Ограничение: синтезированная речь чище живой, а выборка мала (mixed n=2) — итоговая проверка живым голосом.
