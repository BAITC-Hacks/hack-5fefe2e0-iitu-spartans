...
Ожидаемый результат: `Test Files 13 passed`, `Tests 103 passed`, проверка типов без ошибок. Тест словаря
терминов сверяет словарь с данными набора в `data/kit` (раздел 9).
...
| `OPENAI_API_KEY` | ключ для живого режима: распознавание, выбор сценария, синтез речи | используется: `router`, маршрутизатор ядра в `web`, `pnpm stt:eval` |
