...
- **Окно сравнения (раздел 12.1).** Колонка OpenAI проверена живьём (SC17 на казахской реплике, 1,6 с).
  Gemini подключён через OpenAI-совместимый вход и проверен на уровне вызова: запрос доходит до Google, но живой
  ответ модели на площадке не замерен — ключ владельца ограничен по IP, и окно честно показывает `403`.
  С ключом без ограничения колонка заполняется так же, как колонка OpenAI; формат `json_object` у этого входа
  на момент сдачи не подтверждён живым ответом.
